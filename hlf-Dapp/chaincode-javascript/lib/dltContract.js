...
class DltContract extends Contract {
    ...
    // pagination logic
    async getAssetsWithPagination(ctx, args) {
        const startKey="";
        const endKey="";
         args = JSON.parse(args);
		const {iterator, metadata} = await ctx.stub.getStateByRangeWithPagination(startKey, endKey, args['pageSize'], args['bookmark']);
		...
		return {status:true, assets:JSON.stringify(results)};
	}
